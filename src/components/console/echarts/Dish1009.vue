...
export default {
    ...
    methods:{
        getData:function(){
            ...
            this.$api.yye.d1009(params).then(res=>{
                if(res.code == 200)
                {
                    // 满意标签排行
                    this.tags_good_rank.yAxis.data = res.data.good_titles
                    this.tags_good_rank.series[0].data = res.data.good_series
                    var main = this.$refs.echartDemo;
                    let myChart = this.$echarts.init(main);
                    myChart.clear(); //如果图表有修改需求建议加上此方法先清后画
                    myChart.setOption(this.tags_good_rank);
                    // 满意标签占比
                    this.tags_good_pie.series[0].data = res.data.good_series
                    var main2 = this.$refs.echartDemo2;
                    let myChart2 = this.$echarts.init(main2);
                    myChart2.clear(); //如果图表有修改需求建议加上此方法先清后画
                    myChart2.setOption(this.tags_good_pie);
                    // 不满意标签排行
                    this.tags_bad_rank.yAxis.data = res.data.bad_titles
                    this.tags_bad_rank.series[0].data = res.data.bad_series
                    var main3 = this.$refs.echartDemo3;
                    let myChart3 = this.$echarts.init(main3);
                    myChart3.clear(); //如果图表有修改需求建议加上此方法先清后画
                    myChart3.setOption(this.tags_bad_rank);
                    // 不满意标签占比
                    this.tags_bad_pie.series[0].data = res.data.bad_series
                    var main4 = this.$refs.echartDemo4;
                    let myChart4 = this.$echarts.init(main4);
                    myChart4.clear(); //如果图表有修改需求建议加上此方法先清后画
                    myChart4.setOption(this.tags_bad_pie);
                }
                this.loading = false;
            });
        },
