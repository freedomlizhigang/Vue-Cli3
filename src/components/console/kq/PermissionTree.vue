    ...
    <div class="permission-tree">
        <Row>
            ...
            <div class="permission-left">
                ...
                <Tree :data="permissiontree" ref="permissiontree" :render="renderMenu" class="mt10"></Tree>
            </div>
            </Col>
            <Col :xs="0" :sm="1" >
                <div class="permission-border"></div>
            </Col>
            <Col :xs="24" :sm="16">
    ...
    export default {
        ...
        methods: {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.menuData.id == 0) {
                            this.$api.permission.create(this.menuData).then(res=>{
                                if (res.code == 200) {
                        ...
                        {
                            this.$api.permission.edit(this.menuData).then(res=>{
                                if (res.code == 200) {
                                    // 更新左侧的树
                                    this.getPermissionTree();
            ...
            getPermissionTree:function(){
                var self = this
                this.$api.permission.tree().then(res=>{
            ...
            // 单条选中
            detail (data) {
                this.$api.permission.select().then(res=>{
                    if (res.code == 200) {
                        this.menuSelect = res.data;
                    }
                });
                this.$api.permission.detail({'menu_id':data.menu_id}).then(res=>{

                    if (res.code == 200) {
                        res.data.display = res.data.display == 1 ? true : false;
                        this.menuData = res.data;
            ...
            remove (root, node, data) {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        this.$api.permission.remove({'menu_id':data.menu_id}).then(res=>{
                            if(res.code == 200)
                            {
                                // 更新左侧的树
                                this.getPermissionTree();
